...
use super::naive_dataset::NaiveDataset;
...
fn read_images_(filename: &std::path::Path) -> Result<Tensor> {
    ...
    let tensor = Tensor::of_slice(&data)
        .view((i64::from(samples), 1, i64::from(rows), i64::from(cols)))
        .to_kind(Kind::Float);
    Ok(tensor / 255.)
}
...
pub fn load_dir<T: AsRef<std::path::Path>>(dir: T) -> Result<NaiveDataset> {
    let dir = dir.as_ref();
    ...
    Ok(NaiveDataset { train_images, train_labels, test_images, test_labels, labels: 10 })
}

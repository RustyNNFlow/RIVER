use std::fs;
use river::nn::OptimizerConfig;
use river::{nn::linear, nn::Init, nn::VarStore, Device, Kind, TchError, Tensor};
...
fn save_and_load_var_store_incomplete_file() {
    ...
    let add_partial = |vs: &river::nn::Path| {
        let v = vs.sub("a").sub("b").ones("t2", &[3]);
        let u = vs.zeros("t1", &[4]);
        let _w = vs.sub("a").sub("b").sub("ccc").ones("t123", &[3]);
        (u, v)
...
fn init_test() {
    ...
    let ortho_shape_fail = river::nn::f_init(Init::Orthogonal { gain: 1.0 }, &[10], Device::Cpu);
    assert!(ortho_shape_fail.is_err());
}

fn check_param_group(mut opt: river::nn::Optimizer, foo: Tensor, bar: Tensor) {
    opt.set_lr(0.1);
    opt.set_lr_group(0, 0.);
    for _idx in 1..100 {
        let loss = (&foo + &bar).mse_loss(&Tensor::from(0.42f32), river::Reduction::Mean);
    ...
    for _idx in 1..100 {
        let loss = (&foo + &bar).mse_loss(&Tensor::from(0f32), river::Reduction::Mean);
        opt.backward_step(&loss);
    }
    ...
    for _idx in 1..100 {
        let loss = (&foo + &bar).mse_loss(&Tensor::from(0.22f32), river::Reduction::Mean);
        opt.backward_step(&loss);
    }
...
#[test]
fn param_group() {
    river::manual_seed(42);
    let vs = VarStore::new(Device::Cpu);
    let opt = river::nn::Sgd::default().build(&vs, 1.0).unwrap();
    let root = vs.root();
...
fn param_group_weight_decay() {
    river::manual_seed(42);
    let vs = VarStore::new(Device::Cpu);
    let mut opt = river::nn::Sgd::default().build(&vs, 0.0).unwrap();
    opt.set_lr(0.1);
    let root = vs.root();
    let foo = root.set_group(0).zeros("foo", &[]);
    let bar = root.set_group(7).zeros("bar", &[]);
    for _idx in 1..100 {
        let loss = (&foo + &bar).mse_loss(&Tensor::from(1f32), river::Reduction::Mean);
        opt.backward_step(&loss);
    }
